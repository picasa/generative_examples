...
source("R/functions_graph.r")
source("R/functions_ode.r")
...
ggsave(
  ...
  dpi=300, width=format$a2[1], height=format$a2[2], scale=1, units="mm")

# select and save n outputs
ggsave(
  data_collection$plot[[1]] + geom_point(alpha= 1/3), 
  file = "./R/figures/attractors/attractor_test.png",
  dpi=300, width=format$a4[1], height=format$a4[1], scale=1, units="mm")
...
n_plot = 12 # number of parameter combination to be simulated
...
data_collection <- table_collection %>%
  ...
  mutate(xy = future_map(p, ~ iterate(quadratic_map, ., 0, 0, iterations = 1e6))) %>% 
  filter(map_lgl(xy, ~ with(., all(abs(x) + abs(y) < 1e7)))) %>% 
  filter(map_lgl(xy, ~ density_metric(.) %>% pull(cv) < 2)) %>% 
  mutate(xy = map(xy, ~ normalize_xy(.)))
...
data_collection <- data_collection %>%
  ...
  mutate(
    s = walk2(file, plot, ~ ggsave(
      ...
      width=format$a4[1], height=format$a4[1], units="mm"
      ))
  )
...
n_plot = 8       # number of parameter combination to sample
...
value_pattern = c("GWMEASBXSDLY", "XMGMBDILSCFO", "HGUHKHDJQUFN", "VVKLLANMEDGG")
