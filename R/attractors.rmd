...
table_set <- tibble(pattern = 1:30000) %>%
...
plot_set_keep <- data_set %>% filter(density >= 120) %>% render_grid()
plot_set_discard <- data_set %>% filter(density < 120) %>% render_grid()
plot_set_volume <- data_set %>% filter(density >= 200) %>% render_grid()

plot_set_focus <- data_set %>%
  filter(pattern == 23069) %>%
  compute_grid_data(1E6) %>% 
  render_grid() 

ggsave(
  plot_set_keep + facet_wrap(vars(pattern), ncol=20),
  file="./R/figures/attractor_set_keep.png",
  dpi=200, width = 7, height = 7, scale = 2)
...
```{r map_print_multiple}

scale_ink <- scale_color_gradientn(colors = c("white","grey60","#036564", "#033649", "#031634")) 
...
list_selected_evolve <- c(21588, 10004, 1017)
...
data_print <- table_set %>%
  ...
  mutate(
    data = future_map(a, compute_print_data, iteration=3E6, gridsize=1000),
    plot = map(data, render_print)
  )

plot_print <- cowplot::plot_grid(plotlist = rev(data_print$plot), nrow=1)

# AX ratio : 6:8.5
ggsave(
  plot_print,
  file=glue::glue("./R/figures/attractor_set_{name}.png"),
  dpi=300, width=15, height=5, scale=1)
