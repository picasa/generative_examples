...
render_grid <- function(data, color="black", output="facet") {
  ...
  plot <- data %>%
    ggplot(aes(x, y)) +
    geom_point(color = color, size = 0, shape = 20, alpha = 0.1) +
    coord_equal()
  
  switch(output,
    "single" = return(plot),
    "facet" = return(plot + facet_wrap(~ pattern))
  )

}

render_print <- function(data, scale_color = scale_grey) {
  plot <- data %>%
    ggplot(aes(x, y)) +
    geom_point(aes(alpha = sqrt(n), color = log(n)), size = 0, shape = 20) +
    scale_alpha_continuous(range = c(0, 1), limits = c(0, NA)) +
    scale_color + coord_equal()
...
table_set <- tibble(pattern = 1:50000) %>%
...
# cluster analysis for scatterplots or heatmaps based on 2D point density distribution
...
matrix_cluster <- data_cluster %>% 
  select(pattern, heatmap) %>% 
  unnest(heatmap) %>%
  pivot_wider(names_from = c(x, y), values_from = value) %>% 
  select(-pattern) %>% as.matrix()

# cl <- kmeans(matrix_cluster, 3)
cl <- pam(matrix_cluster, 6, pamonce = 5)

data_cluster <- data_cluster %>% mutate(cluster = cl$cluster)
...
data_cluster_subset <- data_cluster %>% filter(density < 100) 
...
plot_cluster <- plot_grid(
  plot_grid(plotlist=data_cluster_subset %>% filter(cluster==1) %>% pull(plot)),
  plot_grid(plotlist=data_cluster_subset %>% filter(cluster==2) %>% pull(plot)),
  plot_grid(plotlist=data_cluster_subset %>% filter(cluster==3) %>% pull(plot)),
  plot_grid(plotlist=data_cluster_subset %>% filter(cluster==4) %>% pull(plot)),
  plot_grid(plotlist=data_cluster_subset %>% filter(cluster==5) %>% pull(plot)),
  plot_grid(plotlist=data_cluster_subset %>% filter(cluster==6) %>% pull(plot)),
  labels=1:6, scale=0.8
)

ggsave(
  plot_cluster,
  file="./R/figures/attractor_set_cluster.png",
  dpi=200, width = 7, height = 10, scale = 2)
...
plot_print <- table_set %>%
  filter(pattern == 27290) %>%
  compute_print_data(a=.$a[[1]], iterations=1E6, gridsize=1000) %>% 
  ggplot(aes(x, y)) + 
  geom_point(aes(alpha = sqrt(n), color = log(n)), size = 0, shape = 20) +
  scale_alpha_continuous(range = c(0, 1), limits = c(0, NA)) +
  scale_colour_gradient(low="white", high="black") +
  coord_equal() + scale_ink

```

```{r print_multiple, eval=FALSE}
table_set <- read_rds("data/attractor_parameter_set.rds")
...
data_print <- table_set %>%
  ...
  mutate(
    data = future_map(a, compute_print_data, iteration=2E6, gridsize=1000),
    plot = map(data, render_print)
  )

write_rds(data_print, glue::glue("data/attractor_set_{name}.rds"), compress="gz")
hash_md5 <- digest::digest(glue::glue("data/attractor_set_{name}.rds"), file=TRUE)
footer <- paste(Sys.Date(), substr(hash_md5, start = 1, stop = 7))

plot_print <- plot_grid(plotlist = rev(data_print$plot), ncol=1) + 
  draw_label(footer, x=0.99, y=0.01, hjust=1, size=6, color="grey40")
...
ggsave(
  ...
  dpi=300, width=210, height=295, scale=2, units="mm")
...
# function to sample a sequence of characters in a list
sample_sequence <- function(n, length, data) {
  data %>%
...
# generate a low resolution sequence list of characters
...
write_rds(data_characters, glue::glue("data/attractor_set_{name}.rds"), compress="gz")
hash_md5 <- digest::digest(glue::glue("data/attractor_set_{name}.rds"), file=TRUE)
footer <- paste(Sys.Date(), substr(hash_md5, start = 1, stop = 7))

# sample different symbol in different sequence length to emulate text
set.seed(1)

data_print <- tibble(character=sample(20:60, size=9, replace = TRUE)) %>% 
  mutate(sequence = map(
    character, sample_sequence,
    data=data_characters$plot, length=60))
