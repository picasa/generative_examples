...
data_cluster_subset <- data_cluster %>% filter(density < 120) 
...
plot_print <- table_set %>%
  ...
  ggplot(aes(x, y)) + 
  geom_point(aes(alpha = log(n), color = log(n)), size = 0) +
  scale_alpha_continuous(range = c(0, 1), limits = c(0, NA)) +
...
data_print <- table_set %>%
  ...
  mutate(
    ...
    plot = map(data, render_print, scale_color=scale_white)
  )
...
data_print <- table_set %>%
  ...
  mutate(
    data = future_map(a, compute_print_data, iteration=2E6, gridsize=1000),
    plot = map(data, render_print, scale_color=scale_grey)
  )
