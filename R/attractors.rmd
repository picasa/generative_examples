...
```{r cluster_analysis_input, eval=FALSE}
...
data_cluster <- table_set %>%
  ...
  mutate(plot = future_map(xy, render_grid, output="single"))

# cluster parameters vectors
n_cluster = 25
matrix_cluster <- data_cluster %>% pull(a, name=pattern) %>% bind_rows() %>% t() 
cl <- pam(matrix_cluster, k=n_cluster, pamonce = 5)
data_cluster <- data_cluster %>% mutate(cluster = cl$cluster)

# similar parameter set do not lead to similar graphs, seems expected...
data_plot_subset <- data_cluster %>% 
  group_by(cluster) %>% 
  slice_sample(n=20) %>% nest() %>% 
  mutate(plot_list=map(data, ~ plot_grid(plotlist = .x$plot)))

plot_cluster <- plot_grid(plotlist = data_plot_subset$plot_list, labels=1:n_cluster, scale=0.8)
...
```{r cluster_analysis_output, eval=FALSE}
...
data_plot_subset <- data_cluster_subset %>% 
  group_by(cluster) %>% 
  slice_sample(n=20) %>% nest() %>% 
  mutate(plot_list=map(data, ~ plot_grid(plotlist = .x$plot)))
...
plot_print <- table_set %>%
  ...
  compute_grid_data(iterations=1E6) %>% unnest(xy) %>% 
  ggplot(aes(x, y)) + 
  geom_point(alpha = 0.05, size = 0) +
  coord_equal() + theme_void() 

# outputs without orbital-patterns are better visualized with an estimation of point density
plot_print <- table_set %>%
  filter(pattern == value_pattern) %>%
  compute_print_data(a=.$a[[1]], iterations=3E6, gridsize=1000) %>% 
  ggplot(aes(x, y)) + 
