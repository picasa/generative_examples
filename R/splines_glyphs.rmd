...
data_script <- tibble(text = seq_text) %>% 
  mutate(plot = future_map(
    text,
    ~ render_script(., data=data_glyphs, ncol=n_col, scale=1.8)))
...
#   width=210, height=210, scale=1, units="mm") 
# 
...
data_glyphs <- data_map %>% 
  mutate(
    variation = 1,
    word = rerun(n(), sample_letters(n = n_chr)),
...
data_script <- tibble(text = seq_text) %>% 
  mutate(plot = future_map(
    text, ~ render_script(., data=data_glyphs, ncol=n_col, scale=scale_chr)))
...
n_points = 4    # number of control points per character
n_chr = 3       # number of elements to be concatenated as new glyphs
n_col = 30      # number of characters per line (default 70)
...
data_glyphs <- data_map %>% 
  mutate(
    variation = 1,
    word = map_chr(character, ~ sample_letters(n = n_chr)),
    layout_word = map(word, ~ layout_word(., data_map)),
...
data_script <- tibble(text = seq_text) %>%
  mutate(plot = future_map(
    text, ~ render_script(
      ., data = data_glyphs, ncol = n_col, scale = scale_chr, o = "v")))
...
file <- glue::glue("text_vertical_", sprintf("%02d", p_seed), sprintf("%02d", p_seed_2))
...
#   plot_script,
#   file=glue::glue("R/figures/splines/svg/{file}.svg"),
#   width=format$a4[1], height=85, scale=1, units="mm")
